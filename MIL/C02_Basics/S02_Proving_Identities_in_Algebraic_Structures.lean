...
theorem add_neg_cancel_right (a b : R) : a + b + -b = a := by
  rw [add_assoc, MyRing.add_right_neg b, add_zero]
...
lemma mul_right_eq_one {a b : G} (h : a * b = a) : b = 1 := by
  rw [<- inv_mul_cancel a, <- one_mul b, <- inv_mul_cancel a, mul_assoc, h]

theorem mul_inv_cancel (a : G) : a * a⁻¹ = 1 := by
  have h : (a * a⁻¹) * (a * a⁻¹) = (a * a⁻¹) := by
    rw [mul_assoc, <- mul_assoc a⁻¹, inv_mul_cancel, one_mul]
  apply mul_right_eq_one h

lemma mul_right_cancel {a b c : G} (h : a * b = c * b) : a = c := by
  sorry

theorem mul_one (a : G) : a * 1 = a := by
  have h : a * 1 * a⁻¹ = a * a⁻¹ := by
    rw [mul_inv_cancel, mul_assoc, one_mul, mul_inv_cancel]
